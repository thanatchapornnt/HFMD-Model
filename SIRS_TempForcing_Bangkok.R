...
out <- as.data.frame(ode(y = y0, times = times,
                         func = SIRS_temp_forcing, parms = pars))
...
print(final_plot)
ggsave("SIRS_TempForcing_Bangkok_plot1.png", plot = final_plot, width = 10, height = 10, dpi = 300)
...
p4 <- ggplot() +
  ...
  scale_x_continuous(limits = c(0, MaxYears), breaks = 0:MaxYears,
                     labels = 2014:(2014 + MaxYears), expand = c(0, 0)) +
  labs(title = "Infectious (I) — Red = model, Black dots = actual cases",
       x = "Year", y = "Proportion") +
  theme_minimal() +
  theme(panel.border = element_rect(fill = NA, color = "black"),
        axis.text.x  = element_text(angle = 45, hjust = 1))

final_plot2 <- (p1 / p2 / p3 / p4) +
  plot_annotation(
    title    = "HFMD SIRS Simulation — Bangkok (2014–2022)",
    subtitle = "Blue = Semester 1 | Light blue = Semester 2 | Orange = Break\nRed line = SIRS model | Black dots = actual reported cases"
  )

print(final_plot2)
ggsave("SIRS_TempForcing_Bangkok_plot2.png", plot = final_plot2, width = 10, height = 13, dpi = 300)
